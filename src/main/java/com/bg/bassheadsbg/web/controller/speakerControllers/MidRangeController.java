...
public class MidRangeController {
    ...
    public String addMidRange(Model model) {
        if (!model.containsAttribute("addMidRangeDTO")) {
            model.addAttribute("addMidRangeDTO", midRangeService.createNewDevice());
        }
    ...
    public String rankings(Model model) {
        model.addAttribute("allDevices", midRangeService.getAllDeviceSummarySorted());
        return "speakers/midrange-all";
    }

    @PostMapping("/like/{id}")
    public String like(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
        boolean likeSpeakerSuccess = midRangeService.likeDevice(id);
