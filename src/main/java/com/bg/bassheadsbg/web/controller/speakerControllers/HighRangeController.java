...
public class HighRangeController {
    ...
    public String addHighRange(Model model) {
        if (!model.containsAttribute("addHighRangeDTO")) {
            model.addAttribute("addHighRangeDTO", highRangeService.createNewDevice());
        }
    ...
    public String rankings(Model model) {
        model.addAttribute("allDevices", highRangeService.getAllDeviceSummarySorted());
        return "speakers/highrange-all";
    }

    @PostMapping("/like/{id}")
    public String like(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
        boolean likeSpeakerSuccess = highRangeService.likeDevice(id);
